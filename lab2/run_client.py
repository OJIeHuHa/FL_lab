...
def run_client():
    ...
    if args.dataset == "fashionmnist":
        model = CustomFashionModel(strategy=args.strategy)
    elif args.dataset == "cifar10":
        print("===CIFAR===")
        model = CustomCifarModel(strategy=args.strategy)
    else:
        print("Invalid dataset name")
        return
    
    client = CustomClient(model, train_loader, val_loader, device, args.cid)
